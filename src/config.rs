...
pub fn svc_prop<S: AsRef<str>>(prop: S, svc: S) -> Result<String, SdcConfigError> {
    ...
    if !output.status.success() {
        return Err(SdcConfigError::BadExitStatus(
            "svcprop".to_string(),
            output.status,
        ));
    }
...
fn get_sdc_config_filename() -> Result<SdcConfigPaths, SdcConfigError> {
    ...
    if Path::new(&cn_config).exists() {
        if Path::new(&sdc_config).exists() {
            ...
            let _ = writeln!(
                &mut dev_msg,
                "WARNING: ignoring config at {} since we have {}",
...
fn sdc_config_to_map(config: &SdcConfigPaths, headnode: bool) -> Result<SdcConfig, SdcConfigError> {
